...
df1 = dataset['validation_mismatched'] 
df2 = dataset['validation_matched']
# Split the dataset into train, validation, and test sets
train_size = 0.8
train_dataset = dataset['train'].shuffle().select(range(int(len(dataset['train']) * train_size)))
valid1_dataset =  dataset["validation_matched"], dataset["validation_mismatched"]
valid_dataset = concatenate_datasets(valid1_dataset)
test1 = dataset["test_matched"], dataset["test_mismatched"]
test_dataset = concatenate_datasets(test1)
...
def get_logits_with_keys(model, ids, mask, token_type_ids):
    output = model(ids, mask, token_type_ids, key_optim=False)
    logits = output
    keys = model.enc_with_bottleneck.vq.key_embed
    return logits, keys
def student_logits_keys(model, ids, mask, key_optim=False):
    output = model(ids, mask, key_optim=key_optim)
    logits = output
    keys = None if key_optim else model.enc_with_bottleneck.vq.key_embed
    return logits, keys
...
for epoch in train_iterator:
    ...
    for batch_idx, data in enumerate(epoch_iterator):
        ...
        teacher_logits, teacher_keys = get_logits_with_keys(teacher_model, ids, mask, token_type_ids)

        student_logits, student_keys = student_logits_keys(student_model,ids, mask,key_optim=False)

        # Carry over the keys from the teacher to the student
        for student_key, teacher_key in zip(student_keys, teacher_keys):
            student_key.data.copy_(teacher_key.data)        
    ...
    glue_metric = glue_compute_metrics("mnli", predicted_labels=y_pred, labels=y_true)
